...
(function () {
  ...
  const agentTab = document.getElementById(AGENT_ID);
  const agentDialog = document.getElementById(getDialogId(AGENT_ID));
  ...
  let clickLeaveExitHandler = evt => {
    ...
    if (type === 'leave') {
      ...
      if (to !== AGENT_ID) {
        chat.showRemoteMessage(chat.serverTab.id, user.LEAVE_CHAT_WITH_AGENT);
      }

      agentTab.id = AGENT_ID;
      agentTab.textContent = 'Ask a guestion';
      agentDialog.id = getDialogId(AGENT_ID);
  ...
  let handleInterlocutorMessage = function (message) {
    ...
    agentTab.id = id;
    agentTab.textContent = name;
    agentDialog.id = getDialogId(id);
  ...
  let handleLeaveMessage = function(message) {
    ...
    chat.showRemoteMessage(chat.serverTab.id, user.INTERLOCUTOR_LEAVE);
  }
